...
function danhmucid($data,$danhmuccha=0,$str=" ",$select=0){
    foreach ($data as $val){
        $id = $val['id'];
        $name = $val["tendanhmuc"];
        if($val["danhmuccha_id"] == $danhmuccha) {
            if($select != 0 && $id == $select){
                echo "<option value='$id' selected='selected'>$str $name</option>";
            }   else{
                echo "<option value='$id'>$str $name</option>";
            }
            danhmucid($data,$id,$str.$name." -- ",$select);
        }
    }
}

function donhang($dh,$str=" "){
    foreach ($dh as $val){
        $name = $val["tinhtrang"];
        if($val["tinhtrang"] == 1) {
            echo "<option value='$name'>Chưa giao</option>";
        }
        if($val["tinhtrang"] == 2) {
            echo "<option value='$name'>Đã giao</option>";
        }
        if($val["tinhtrang"] == 3) {
            echo "<option value='$name'>Huỷ</option>";
        }
    }
}
?>
